/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import { remap as remap$ } from "../../../lib/primitives.js";
import { ClosedEnum } from "../../types/enums.js";
import * as shared from "../shared/index.js";

export const UpdateAtsInterviewQueryParamFields = {
  Id: "id",
  CreatedAt: "created_at",
  UpdatedAt: "updated_at",
  CandidateId: "candidate_id",
  JobId: "job_id",
  ApplicationId: "application_id",
  UserIds: "user_ids",
  Status: "status",
  StartAt: "start_at",
  EndAt: "end_at",
  Location: "location",
  ExternalEventXref: "external_event_xref",
  Raw: "raw",
} as const;
export type UpdateAtsInterviewQueryParamFields = ClosedEnum<
  typeof UpdateAtsInterviewQueryParamFields
>;

export type UpdateAtsInterviewRequest = {
  atsInterview: shared.AtsInterview;
  /**
   * ID of the connection
   */
  connectionId: string;
  /**
   * Fields to return
   */
  fields?: Array<UpdateAtsInterviewQueryParamFields> | undefined;
  /**
   * ID of the Interview
   */
  id: string;
  /**
   * Raw parameters to include in the 3rd-party request. Encoded as a URL component. eg. raw parameters: foo=bar&zoo=bar -> raw=foo%3Dbar%26zoo%3Dbar
   */
  raw?: string | undefined;
};

/** @internal */
export const UpdateAtsInterviewQueryParamFields$outboundSchema: z.ZodNativeEnum<
  typeof UpdateAtsInterviewQueryParamFields
> = z.nativeEnum(UpdateAtsInterviewQueryParamFields);

/** @internal */
export type UpdateAtsInterviewRequest$Outbound = {
  AtsInterview: shared.AtsInterview$Outbound;
  connection_id: string;
  fields?: Array<string> | undefined;
  id: string;
  raw?: string | undefined;
};

/** @internal */
export const UpdateAtsInterviewRequest$outboundSchema: z.ZodType<
  UpdateAtsInterviewRequest$Outbound,
  z.ZodTypeDef,
  UpdateAtsInterviewRequest
> = z.object({
  atsInterview: shared.AtsInterview$outboundSchema,
  connectionId: z.string(),
  fields: z.array(UpdateAtsInterviewQueryParamFields$outboundSchema).optional(),
  id: z.string(),
  raw: z.string().optional(),
}).transform((v) => {
  return remap$(v, {
    atsInterview: "AtsInterview",
    connectionId: "connection_id",
  });
});

export function updateAtsInterviewRequestToJSON(
  updateAtsInterviewRequest: UpdateAtsInterviewRequest,
): string {
  return JSON.stringify(
    UpdateAtsInterviewRequest$outboundSchema.parse(updateAtsInterviewRequest),
  );
}
