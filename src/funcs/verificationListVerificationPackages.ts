 ...
 * List all packages
 */
export function verificationListVerificationPackages(
  client: UnifiedToCore,
  request: operations.ListVerificationPackagesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    Array<shared.VerificationPackage>,
    | UnifiedToError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListVerificationPackagesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, z.array(shared.VerificationPackage$inboundSchema)),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
