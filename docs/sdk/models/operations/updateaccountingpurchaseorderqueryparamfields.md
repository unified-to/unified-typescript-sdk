# UpdateAccountingPurchaseorderQueryParamFields

## Example Usage

```typescript
import { UpdateAccountingPurchaseorderQueryParamFields } from "@unified-api/typescript-sdk/sdk/models/operations";

let value: UpdateAccountingPurchaseorderQueryParamFields = "raw";
```

## Values

```typescript
"id" | "created_at" | "updated_at" | "posted_at" | "contact_id" | "account_id" | "currency" | "total_amount" | "shipping_address" | "billing_address" | "status" | "lineitems" | "raw"
```