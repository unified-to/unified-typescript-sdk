...
async function run() {
  const result = await unifiedTo.group.getClubsGroup({
    connectionId: "<id>",
    id: "<id>",
  });
...
**Promise\<[shared.ClubsGroup](../../sdk/models/shared/clubsgroup.md)\>**
...
async function run() {
  const result = await unifiedTo.group.listClubsGroups({
    connectionId: "<id>",
  });
...
**Promise\<[shared.ClubsGroup[]](../../models/.md)\>**
