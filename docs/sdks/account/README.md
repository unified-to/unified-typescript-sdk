...
async function run() {
  const result = await unifiedTo.account.getAccountingAccount({
    connectionId: "<id>",
    id: "<id>",
  });
...
async function run() {
  const result = await unifiedTo.account.listAccountingAccounts({
    connectionId: "<id>",
  });
...
async function run() {
  const result = await unifiedTo.account.patchAccountingAccount({
    accountingAccount: {
      ...
      customerDefinedCode: "quo",
      description: "Spoliatio comedo vilitas harum cupiditate.",
      id: "d7735b01-7e01-4ee6-a471-32f820c37611",
      isPayable: true,
      name: "Electronic Aluminum Tuna",
      status: "ARCHIVED",
      taxonomy: [
      ...
      type: "BANK",
      updatedAt: new Date("2023-01-03T05:40:29.613Z"),
    },
    connectionId: "<id>",
    id: "<id>",
  });
...
async function run() {
  const result = await unifiedTo.account.removeAccountingAccount({
    connectionId: "<id>",
    id: "<id>",
  });
